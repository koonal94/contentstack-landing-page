...
export async function fetchCompany(forcedEntryUid = null, ignoreStoredUid = false) {
  const stack = getStack()
  if (!stack) return null
  
  let contentType = 'company'
  const locale = 'en-us'
  ...
  if (!ignoreStoredUid && typeof window !== 'undefined') {
    try {
      ...
      if (ctFromUrl && ctFromUrl === 'company') {
  ...
  if (entry && typeof window !== 'undefined') {
    ...
    if (inIframe || hasLivePreviewParams || (livePreviewEnabled && (hasContentstackReferrer || isLaunchDomain))) {
      try {
        ...
        if (!entry.fields && (entry.hero || entry.stats || entry.values || entry.footer)) {
          entry.fields = {}
          const groupNames = ['hero', 'stats', 'values_section', 'values', 'cta_section', 'footer']
        ...
        if (typeof addEditableTags === 'function') {
          ...
          if (editTags && currentEntryUid) {
            ...
            const groupFields = {
              ...
              stats: [], // Array field - handled at group level
              values_section: ['heading'],
              values: [], // Array field - handled at group level
              cta_section: ['heading', 'description', 'button_text', 'button_link'],
              footer: ['link_groups'],
            }
            ...
            Object.keys(groupFields).forEach(groupName => {
              if (editTags[groupName]) {
                ...
                // For array fields like stats and values, create group-level tag
                if ((groupName === 'stats' || groupName === 'values') && !entryData[groupName].$['data-cslp']) {
                ...
                if ((groupName === 'stats' || groupName === 'values') && !entry.$[groupName]['data-cslp']) {
...
export function mapCompany(entry) {
  if (!entry) return null
  
  let fields = entry.fields
  
  if (!fields && (entry.hero || entry.stats || entry.values || entry.footer)) {
    fields = entry
  } else if (!fields) {
    return null
  }
  ...
  return {
    ...
    stats: (fields?.stats || []).map((s) => ({
      number: s?.number,
      label: s?.label,
    })),
    valuesSection: {
      heading: fields?.values_section?.heading,
    },
    values: (fields?.values || []).map((v) => ({
      title: v?.title,
      description: v?.description,
      icon: v?.icon || 'Target',
      color: v?.color || 'from-blue-500 to-blue-600',
    })),
    ctaSection: {
      heading: fields?.cta_section?.heading,
      description: fields?.cta_section?.description,
      buttonText: fields?.cta_section?.button_text,
      buttonLink: fields?.cta_section?.button_link?.href || fields?.cta_section?.button_link || '/get-started',
    },
    footer: {
