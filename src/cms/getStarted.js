...
export async function fetchGetStarted(forcedEntryUid = null, ignoreStoredUid = false) {
  ...
  if (!ignoreStoredUid) {
    try {
      ...
      if (hashMatch && !entryUid && (inIframe || forcedEntryUid)) {
        ...
        console.log('[GET_STARTED] Found entry UID in hash:', entryUid)
      }
    } catch {}
  }
      ...
      try {
        ...
        if (is422Error && entryUid && typeof window !== 'undefined') {
          ...
          try {
            ...
            if (localContentType === 'get_started') {
              localStorage.removeItem('contentstack_entry_uid')
            }
          } catch {}
          // Clear entryUid so we fall through to query published entries
          entryUid = ''
          console.warn('[GET_STARTED] Entry UID invalid (422), falling back to query published entries')
        } else {
  ...
  if (!entry && !inIframe) {
    try {
      ...
      if (!entry) {
        ...
        console.warn('[GET_STARTED] No published entries found. Check environment variable matches published environment.')
      }
    } catch (e) {
      console.error('[GET_STARTED] Query for published entries failed:', e)
    }
  }
